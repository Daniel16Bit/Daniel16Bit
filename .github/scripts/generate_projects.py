#!/usr/bin/env python3
"""
Gera o painel de projetos (projects.svg + projects-light.svg) na estetica Kali.

Le projects.json (curado por voce) + dados ao vivo do GitHub mesclados pelo
workflow (fetch_data.py -> merged.json). Um SVG, grade 2 colunas de mini-cards
de terminal. Para adicionar/remover/reordenar projetos, edite projects.json.
"""
...
GREEN = STROKE = STROKE_HI = STROKE_LO = BARLINE = RING_BG = PILL_BG = PILL_STROKE = MONO_TX = None
DONUT_COLORS = []
...
def card(p, x, y, idx):
    ...
    a(f'<text x="16" y="19" font-size="10" fill="{MUTED}"><tspan fill="{GREEN}">&#8226;</tspan> {esc(repo)}</text>')
    ...
    try:
        ...
        pass
    if days <= 14:
        a(f'<circle cx="{CARD_W-16}" cy="15" r="3.5" fill="{GREEN}">'
    ...
    a(f'<text x="68" y="61" font-size="17" font-weight="700" fill="{TEXT}">{name}'
      f'<tspan fill="{GREEN}">_<animate attributeName="opacity" values="1;0;1" dur="1.2s" '
    ...
    for tag in (p.get("tags") or [])[:3]:
        ...
        a(f'<text x="{tx + tw/2:.0f}" y="130" text-anchor="middle" font-size="9.5" fill="{GREEN}">{esc(tag)}</text>')
        tx += tw + 7
...
def build(projects, theme="dark"):
    ...
    a(f'<defs><linearGradient id="{gid}" x1="0" y1="0" x2="1" y2="0">'
      f'<stop offset="0" stop-color="{GREEN}"><animate attributeName="stop-color" values="{GREEN};{CYAN};{GREEN}" dur="10s" repeatCount="indefinite"/></stop>'
      f'<stop offset="1" stop-color="{CYAN}"><animate attributeName="stop-color" values="{CYAN};{GREEN};{CYAN}" dur="10s" repeatCount="indefinite"/></stop>'
      '</linearGradient></defs>')
    a(f'<text x="{MARGIN+2}" y="18" font-size="11" letter-spacing="2" fill="{GREEN}">PROJECTS.LIST</text>')
    a(f'<text x="{MARGIN+130}" y="18" font-size="10" fill="{DIM}">root@kali:~$ ls ./projects --all</text>')
